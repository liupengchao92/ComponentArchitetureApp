...
class SettingActivity : BaseBindingActivity<ActivitySettingBinding>() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.followSystemSwitchButton.isChecked = DarkModeUtils.isFollowSystemDarkMode()

        binding.followSystemSwitchButton.setOnCheckedChangeListener { buttonView, isChecked ->
            if (isChecked){
                DarkModeUtils.applySystemMode()
            }
        }

        binding.darkSwitchButton.isChecked = DarkModeUtils.isDarkMode(this)
        //是否是暗黑模式
        binding.darkSwitchButton.setOnCheckedChangeListener { buttonView, isChecked ->

            if (isChecked) {
                DarkModeUtils.applyNightMode()
            } else {
                DarkModeUtils.applyDayMode()
            }
        }
